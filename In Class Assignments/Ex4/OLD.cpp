...
Type objective_function<Type>::operator() ()
{
  ...
  matrix<Type> omega(Nyear + Nproj, Nclass);
  //vector<Type> Int_Pop(Nclass);
  ...
  for (int Iyear=0; Iyear<Nyear+Nproj; Iyear++)
    ...
    {
      F(Iyear, Iclass) = S(Iclass) * exp(LogFullF(Iyear)); //full fishing mortality matrix
      Z(Iyear, Iclass) = M + F(Iyear, Iclass); 
      omega(Iyear, Iclass) = exp(-Z(Iyear, Iclass));
    }
  ...
  for (int i=0; i<Nyear; i++){
    SS += pow((log(CW(i)) - log(CWObs(i))), 2);
  }
  
  LikeCatch = 1/(2*pow(0.05,2))*SS;
  ...
  for (int t=0;t<Nyear;t++){
    SS2 += (log(BioIndex(t)) - log(q*BioPred(t)));
  }
  
  LikeBio = 1/(2*pow(0.05,2))*SS2;
